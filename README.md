...
대규모 언어 모델(LLM)이 학습할때에 포함되지 못은 특정 영역의 데이터(domain specific data)는 [RAG (Retrieval Augmented Generation) ](https://docs.aws.amazon.com/ko_kr/sagemaker/latest/dg/jumpstart-foundation-models-customize-rag.html)와 같이 외부 문서 저장소의 Question에 관련된 문서를 읽어와서 Prompt에 활용하는 방식으로 강화될 수 있습니다. Amazon Kendra는 자연어 처리 및 고급 기계 학습 알고리즘을 사용하여 데이터에서 검색 질문에 대한 답변을 얻는 지능형 검색 서비스로서 대규모 언어 모델에 RAG를 구현할 때 유용하게 활용될 수 있습니다. 
